...
import type { GraphicsEntryType } from '@editor/stores/graphics-editor-session';
...
export function buildSaveLayerItems(
  ...
  const textureItem: SaveLayerItem = {
    ...
    isValid: (context.textureValidation?.success ?? false) && (context.imageDirty ? (context.imageValidation?.success ?? false) : true),
